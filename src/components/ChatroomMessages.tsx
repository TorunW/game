...
function ChatroomMessages() {
  ...
  const activeTurn = useAppSelector(
    (state) => state.chatroomMessages.turnIsActive
  );
  const gameOver = useAppSelector((state) => state.chatroomMessages.gameOver);
  const isWinner = useAppSelector((state) => state.chatroomMessages.isWinner);
  ...
  if (gameOver === true) {
    gameOverMessage = (
      <div>{isWinner === true ? <p>YOU WON!</p> : <p>YOU LOST!</p>}</div>
    );
  }
    ...
    <div>
      ...
      {activeTurn === true ? (
        <div className='select-number-container'>
          <button
            className='select-number number-1'
            onClick={() => onSendNumber(-1)}
            value='-1'
          >
            -1
          </button>
          <button
            className='select-number number0'
            onClick={() => onSendNumber(0)}
            value='0'
          >
            0
          </button>
          <button
            className='select-number number1'
            onClick={() => onSendNumber(+1)}
            value='+1'
          >
            +1
